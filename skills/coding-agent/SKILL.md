...
metadata: {"clawdbot":{"emoji":"🧩","requires":{"anyBins":["claude","codex","opencode","pi","droid"]}}}
---
...
Interactive terminal apps need a pseudo-terminal (PTY) to work correctly. Without PTY, you'll get broken output, missing colors, or the agent may hang.

**Use `pty:true`** when running interactive agents:
- `codex`
- `claude`
- `opencode`
- `pi`
...
Notes:
- `droid exec` is **non-interactive** (one-shot) and typically **does not** require PTY.
...
### Droid Exec (planning / review)

`droid exec` is great for **plans** and **read-only reviews** (default is spec / read-only).

```bash
# Read-only plan
bash pty:true workdir:~/project command:"droid exec --cwd ~/project -m claude-opus-4-5-20251101 'Create a step-by-step plan. Do not modify files.'"

# If you *explicitly* want edits/commands:
# droid exec --auto low|medium|high "..."
```
...
```bash
# Interactive TUI (PTY required)
bash pty:true workdir:~/project command:"droid"
```
