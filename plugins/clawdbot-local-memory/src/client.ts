...
import { detectCategory, type MemoryCategory, type MemoryEntry, type ProfileResult } from "./memory.js";
...
async function embeddingFetch(endpoint: string, method: string, body?: unknown): Promise<Response> {
...
export class LocalMemoryClient {
  ...
  private profile: ProfileResult = { static: [], dynamic: [] };
  ...
  private async checkEmbeddingService(): Promise<boolean> {
    if (this.embeddingAvailable !== null) return this.embeddingAvailable;
    try {
  ...
  async search(query: string, limit = 5): Promise<SearchResult[]> {
    ...
    if (await this.checkEmbeddingService()) {
      try {
        ...
        if (resp.ok) {
          const data = await resp.json() as EmbeddingQueryResult;
  ...
  async forgetByQuery(query: string): Promise<{ success: boolean; message: string }> {
  ...
  private generateProfileFromMemories(): ProfileResult {
    const now = Date.now();
  ...
  async updateProfile(staticFacts: string[], dynamicFacts: string[]): Promise<void> {
    await this.ensureLoaded();
  ...
  async addToProfile(fact: string, isStatic: boolean): Promise<void> {
    await this.ensureLoaded();
  ...
  async syncToVectorDb(): Promise<{ synced: number; failed: number }> {
    await this.ensureLoaded();
    if (!(await this.checkEmbeddingService())) {
