...
export const signalOnboardingAdapter = {
    ...
    configure: async ({ cfg, runtime, prompter, accountOverrides, shouldPromptAccountIds, options, }) => {
        ...
        if (!cliDetected) {
            await prompter.note("signal-cli not found. Install it, then rerun this step or set signal.cliPath.", "Signal");
        }
                    ...
                    signal: {
                        ...
                        accounts: {
                            ...
                            [signalAccountId]: {
                                ...
                                enabled: next.signal?.accounts?.[signalAccountId]?.enabled ?? true,
                                account,
                                cliPath: resolvedCliPath ?? "signal-cli",
                            },
                        },
