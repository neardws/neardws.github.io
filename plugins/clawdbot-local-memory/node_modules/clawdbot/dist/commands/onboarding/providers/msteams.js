...
function setMSTeamsDmPolicy(cfg, dmPolicy) {
    const allowFrom = dmPolicy === "open"
        ? addWildcardAllowFrom(cfg.msteams?.allowFrom)?.map((entry) => String(entry))
        : undefined;
    return {
        ...cfg,
        msteams: {
...
export const msteamsOnboardingAdapter = {
    provider,
    getStatus: async ({ cfg }) => {
    ...
    configure: async ({ cfg, prompter }) => {
        ...
        const hasConfigCreds = Boolean(cfg.msteams?.appId?.trim() &&
