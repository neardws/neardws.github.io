...
export async function applyAuthChoiceAnthropic(params) {
        ...
        if (!storeWithKeychain.profiles[CLAUDE_CLI_PROFILE_ID]) {
            if (process.stdin.isTTY) {
                ...
                if (runNow) {
                    ...
                    if (res.error) {
                        await params.prompter.note(`Failed to run claude: ${String(res.error)}`, "Claude setup-token");
                    }
                }
            }
            else {
            ...
            if (!refreshed.profiles[CLAUDE_CLI_PROFILE_ID]) {
                await params.prompter.note(process.platform === "darwin"
                    ...
                    : "No Claude Code CLI credentials found at ~/.claude/.credentials.json.", "Claude Code CLI OAuth");
                return { config: nextConfig };
            }
        }
    ...
    if (params.authChoice === "setup-token" || params.authChoice === "oauth") {
        ...
        if (!process.stdin.isTTY) {
            await params.prompter.note("`claude setup-token` requires an interactive TTY.", "Anthropic setup-token");
            return { config: nextConfig };
        }
        const proceed = await params.prompter.confirm({
            message: "Run `claude setup-token` now?",
        ...
        if (res.error) {
            await params.prompter.note(`Failed to run claude: ${String(res.error)}`, "Anthropic setup-token");
            return { config: nextConfig };
        }
        ...
        if (!store.profiles[CLAUDE_CLI_PROFILE_ID]) {
            await params.prompter.note(`No Claude Code CLI credentials found after setup-token. Expected ${CLAUDE_CLI_PROFILE_ID}.`, "Anthropic setup-token");
            return { config: nextConfig };
        }
        nextConfig = applyAuthProfileConfig(nextConfig, {
            profileId: CLAUDE_CLI_PROFILE_ID,
