...
export async function applyAuthChoiceOpenAI(params) {
    ...
    if (params.authChoice === "openai-codex") {
        ...
        try {
            ...
            await params.prompter.note("Trouble with OAuth? See https://docs.clawd.bot/start/faq", "OAuth help");
        }
    ...
    if (params.authChoice === "codex-cli") {
        ...
        if (!store.profiles[CODEX_CLI_PROFILE_ID]) {
            await params.prompter.note("No Codex CLI credentials found at ~/.codex/auth.json.", "Codex CLI OAuth");
            return { config: nextConfig, agentModelOverride };
        }
        nextConfig = applyAuthProfileConfig(nextConfig, {
