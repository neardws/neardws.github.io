...
import { listWhatsAppAccountIds, resolveDefaultWhatsAppAccountId, resolveWhatsAppAccount, } from "../../web/accounts.js";
...
export const whatsappPlugin = {
    ...
    security: {
        resolveDmPolicy: ({ cfg, accountId, account }) => {
            ...
            const useAccountPath = Boolean(cfg.whatsapp?.accounts?.[resolvedAccountId]);
    ...
    actions: {
        ...
        handleAction: async ({ action, params, cfg, accountId }) => {
            ...
            return await handleWhatsAppAction({
                action: "react",
                chatJid: readStringParam(params, "chatJid") ??
        ...
        resolveTarget: ({ to, allowFrom, mode }) => {
            ...
            if (trimmed) {
                ...
                if (!normalizedTo) {
                    ...
                    return {
                        ok: false,
                        error: new Error("Delivering to WhatsApp requires --to <E.164|group JID> or whatsapp.allowFrom[0]"),
                    };
                }
    ...
    status: {
        ...
        buildProviderSummary: async ({ account, snapshot }) => {
            ...
            const linked = typeof snapshot.linked === "boolean"
                ? snapshot.linked
                : authDir
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            return monitorWebProvider(shouldLogVerbose(), undefined, true, undefined, ctx.runtime, ctx.abortSignal, {
                statusSink: (next) => ctx.setStatus({ accountId: ctx.accountId, ...next }),
                accountId: account.accountId,
            });
        },
        loginWithQrStart: async ({ accountId, force, timeoutMs, verbose }) => await (async () => {
        ...
        loginWithQrWait: async ({ accountId, timeoutMs }) => await (async () => {
