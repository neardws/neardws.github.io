...
async function resolveNodeId(opts, query) {
    ...
    if (!q) {
        ...
        throw new Error("node required (use --node or ensure only one connected node is available)");
    }
...
export function registerCanvasCli(program) {
    ...
    canvasCallOpts(a2ui
        ...
        try {
            ...
            if (version === "v0.9") {
                throw new Error("Detected A2UI v0.9 JSONL (createSurface). Clawdbot currently supports v0.8 only.");
            }
            await invokeCanvas(opts, "canvas.a2ui.pushJSONL", { jsonl });
            if (!opts.json) {
                defaultRuntime.log(`canvas a2ui push ok (v0.8, ${messageCount} message${messageCount === 1 ? "" : "s"})`);
            }
        }
        catch (err) {
