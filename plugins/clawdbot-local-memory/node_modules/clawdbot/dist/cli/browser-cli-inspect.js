...
export function registerBrowserInspectCommands(browser, parentOpts) {
    browser
        .command("screenshot")
        ...
        .action(async (targetId, opts, cmd) => {
    ...
    browser
        ...
        .option("--limit <n>", "Max nodes (default: 500/800)", (v) => Number(v))
        ...
        .option("--depth <n>", "Role snapshot: max depth", (v) => Number(v))
        ...
        const configMode = format === "ai" && loadConfig().browser?.snapshotDefaults?.mode === "efficient"
            ? "efficient"
            : undefined;
